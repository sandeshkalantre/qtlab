...
if not(Automatic_sequence_generation):  # If user wants manual sequence generation (Automatic_sequence_generation = False)
    ...
    A1 = np.array([0.0,-10.4,0.0,8,8])*31.25 # Initial amplitudes, 31.25 is factor of attenuation
    delta_A1 = A1[1]/(Seq_length/2.0)
    A2 = np.array([0.0,-68.9,0.0,-4.0,18.98])*31.25*1.2 # Initial amplitudes, 31.25 is factor of attenuation, 1.2 is factor for diagonal pulsing
    ...
    for i in xrange(Seq_length):   # Creating waveforms for all sequence elements
        ...
        #else:
        p.setValuesCH1([100,A1[0]], [50,A1[1]], [50,A1[2]], [300,A1[3]], [100,A1[4]]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
        ...
        #else:
        p.setValuesCH2([100,A2[0]], [50,A2[1]], [50,A2[2]],[300,A2[3]], [100,A2[4]]) # Setting waveform shape for one wavefrom object p in sequence seq for AWG channel 1 - [Time1,Amp1],[Time2,Amp2]...  Time in TimeUnits and Amp in AmpUnits
